...
export default function AdminMenuList({
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 justify-end">
            ...
            <button
              ...
              title="Seed all 181 official Everbloom Café items"
            >
            ...
            return (
              ...
              >
                ...
                <div className="relative h-48 sm:h-52 overflow-hidden bg-gray-100">
                  ...
                  {/* Section Badge */}
                  <div className="absolute bottom-3 left-3 px-2.5 py-1 rounded-xl bg-black/60 backdrop-blur-sm text-[10px] font-bold text-white tracking-wider">
                    {item.category}
                  </div>
                </div>
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-[#6b5c54] line-clamp-2 mt-1">
                    ...
                    {item.tags && item.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2.5">
                        {item.tags.map((tag, idx) => (
                          <span
                            key={idx}
                            className="px-2 py-0.5 rounded-md bg-[#faf7f2] border border-[#e8ded3] text-[9px] font-bold text-[#c88242]"
                          >
                            {tag}
                          </span>
                        ))}
                  ...
                  <div className="pt-3 border-t border-[#e8ded3] flex items-center justify-between">
                    <div>
                      <span className="text-[10px] text-[#6b5c54] block">Price</span>
      ...
      {/* Pagination Bar */}
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white rounded-3xl p-4 sm:p-5 border border-[#e8ded3] shadow-xs">
          <span className="text-xs text-[#6b5c54]">
            Showing <span className="font-bold text-[#2b1810]">{(validCurrentPage - 1) * itemsPerPage + 1}</span> to{" "}
            <span className="font-bold text-[#2b1810]">{Math.min(validCurrentPage * itemsPerPage, totalItems)}</span> of{" "}
            <span className="font-bold text-[#2b1810]">{totalItems}</span> dishes
          </span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 rounded-xl border border-[#e8ded3] bg-[#faf7f2] text-[#2b1810] disabled:opacity-40 disabled:cursor-not-allowed hover:bg-[#e8ded3] transition-colors"
              title="Previous Page"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>

            <span className="text-xs font-bold text-[#6b5c54] px-3">
