...
export default function AmbienceShowcase() {
  const tabs = [
    {
      ...
      image: "https://res.cloudinary.com/p2gsrga3/image/upload/v1789146605/myimg.png",
  ...
  return <section className="section-padding py-20 lg:py-28 bg-[#2a1e1a] text-white relative overflow-hidden">
    ...
    <div
      className={`absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full blur-[120px] pointer-events-none transition-all duration-700 ${activeTab.id === "patio"
        ? "bg-emerald-500/25"
        : activeTab.id === "lounge"
          ? "bg-teal-500/25"
          : "bg-rose-500/30"
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`px-5 sm:px-6 py-3 rounded-full text-xs font-bold transition-all duration-300 flex items-center gap-2.5 backdrop-blur-xl border ${isSelected
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
        ...
        <ScrollReveal variant="left" delay={200} className="lg:col-span-6 flex flex-col justify-center space-y-6">
          <div className="inline-flex items-center gap-2">
            <span className={`w-8 h-[2px] ${activeTab.id === "patio" ? "bg-emerald-400" : activeTab.id === "lounge" ? "bg-teal-400" : "bg-rose-400"
              }`} />
            <span className={`text-[11px] font-extrabold tracking-[0.2em] uppercase ${activeTab.id === "patio" ? "text-emerald-300" : activeTab.id === "lounge" ? "text-teal-300" : "text-rose-300"
              }`}>
              {activeTab.tag}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
            {activeTab.perks.map((perk, i) => (
              ...
              >
                <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${activeTab.id === "patio"
        ...
        <ScrollReveal variant="right" delay={250} className="lg:col-span-6">
          <div className={`relative rounded-3xl overflow-hidden shadow-2xl border transition-all duration-700 group p-2 bg-black/40 backdrop-blur-xl ${activeTab.id === "patio"
            ...
            <div className="relative rounded-2xl overflow-hidden">
              ...
              <div className="absolute bottom-5 left-5 right-5 flex items-center justify-between">
                <span className="px-4 py-1.5 rounded-full bg-black/70 backdrop-blur-md text-[11px] uppercase tracking-wider text-white font-bold border border-white/20 shadow-lg flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full animate-ping ${activeTab.id === "patio" ? "bg-emerald-400" : activeTab.id === "lounge" ? "bg-teal-400" : "bg-rose-400"
                    }`} />
                  {activeTab.label}
                </span>
